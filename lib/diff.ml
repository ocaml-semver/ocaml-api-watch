...
let rec type_expr ~typing_env ?(ref_params = [])
    ?(cur_params = []) reference current =
  let expanded_ref = Typing_env.fully_expand_type_expr ~typing_env
      ~type_expr:reference
  in
  let expanded_cur = Typing_env.fully_expand_type_expr ~typing_env
      ~type_expr:current
  in
