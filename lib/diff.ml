...
and diff_map ~(diff_one : 'a -> 'a -> 'diff option) ~(ref_map : 'a String_map.t)
  ...
  let same_map, diff_map =
    String_map.merge
      (fun _ ref_opt cur_opt ->
        match (ref_opt, cur_opt) with
        | None, None -> None
        | Some ref, None -> Some (Changed (Added ref))
        | None, Some cur -> Some (Changed (Removed cur))
        | Some ref, Some cur -> (
            match diff_one ref cur with
            | None -> Some (Same ref)
            | Some diff -> Some (Changed (Modified diff))))
      ref_map cur_map
    |> String_map.partition (fun _ v ->
           match v with Same _ -> true | Changed _ -> false)
  in
  {
    same_map =
      String_map.map
        (fun i -> match i with Same i -> i | _ -> assert false)
        same_map;
    changed_map =
      String_map.map
        (fun i -> match i with Changed i -> i | _ -> assert false)
        diff_map;
  }
...
and cstr ~typing_env ~ref_params ~cur_params reference current =
  ...
  | _ -> Some (Atomic_cstr { reference; current })
...
and tuple_type ~typing_env ~ref_params ~cur_params ~reference ~current =
  if
    ...
    && List.for_all2
         (fun ref_type cur_type ->
           Option.is_none
             (type_expr ~typing_env ~ref_params ~cur_params ref_type cur_type))
         reference current
  then Same reference
  else
    Changed
