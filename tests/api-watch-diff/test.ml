...
let%expect_test "Resolve alias types in diff_interface" =
  ...
  List.iter
    (function
      | Api_watch_diff.Value (name, Api_watch_diff.Modified (ref_vd, curr_vd))
        ->
          Format.printf "Modified Value %s:\n" name;
          Format.printf "From Reference:\n";
          Printtyp.value_description (Ident.create_local name)
            Format.std_formatter ref_vd;
          Format.print_newline ();
          Format.printf "To Current:\n";
          Printtyp.value_description (Ident.create_local name)
            Format.std_formatter curr_vd;
          Format.print_newline ()
      | _ -> ())
    result;
  [%expect
